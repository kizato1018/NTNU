...
ColorImage Camera::Render(const std::vector<Object> &objects) {
    ...
    for(int y = 0; y < _height; y++) {
        for (int x = 0; x < _width; x++) {
            ...
            vec3 intersection;
            bool hit = _ray_trace(ray, objects, intersection);
...
ColorImage Camera::Render(const std::vector<Object> &objects, const Light &light) {
    ColorImage img;
    ...
    for(int y = 0; y < _height; y++) {
        for (int x = 0; x < _width; x++) {
            ...
            vec3 intersection;
            Pixel p;
            bool hit = _ray_trace(ray, light, objects, intersection, p);
